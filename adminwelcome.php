...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
    ...
    <a href="adminwelcome.php" class="w3-bar-item w3-button w3-padding w3-blue"><i class="fa fa-users fa-fw"></i>  Home Page</a>
...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <div class="w3-panel">
    <div class="w3-row-padding" style="margin:0 -16px">
      <div class="w3-third">
        <h5>Data Science</h5>
        <img src="Datascience.png" style="width:100%" >
      </div>
      <div class="w3-twothird">
        <h5>Subjects</h5>
        <table class="w3-table w3-striped w3-white">
        <tr>
            <td><i class="fa fa-book w3-text-blue w3-large"></i></td>
            <td>Subject</td>
            <td><i>Books Count</i></td>
          </tr>
          <tr>
            <td><i class="fa fa-book w3-text-blue w3-large"></i></td>
          ...
          <tr>
			...
			$sql = 'SELECT Boo.* FROM DSI_Books Boo 
						JOIN DSI_Subjects_Books SB ON SB.Book_ID=Boo.Book_ID 
						JOIN DSI_Subjects Sub ON Sub.Sub_ID=SB.Sub_ID
						WHERE Sub.Sub_Term ="Data Analytics"';
			$query = mysqli_query($db, $sql);
			if (!$query) {
				die ('SQL Error: ' . mysqli_error($db));
			}
          ...
          <tr>
			...
			$sql = 'SELECT Boo.* FROM DSI_Books Boo 
						JOIN DSI_Subjects_Books SB ON SB.Book_ID=Boo.Book_ID 
						JOIN DSI_Subjects Sub ON Sub.Sub_ID=SB.Sub_ID
						WHERE Sub.Sub_Term ="Data Mining"';
			$query = mysqli_query($db, $sql);
			if (!$query) {
				die ('SQL Error: ' . mysqli_error($db));
			}
          ...
          <tr>
			...
			$sql = 'SELECT Boo.* FROM DSI_Books Boo 
						JOIN DSI_Subjects_Books SB ON SB.Book_ID=Boo.Book_ID 
						JOIN DSI_Subjects Sub ON Sub.Sub_ID=SB.Sub_ID
						WHERE Sub.Sub_Term ="Machine Learning"';
			$query = mysqli_query($db, $sql);
			if (!$query) {
				die ('SQL Error: ' . mysqli_error($db));
			}
          ...
          <tr>
			...
			$sql = 'SELECT Boo.* FROM DSI_Books Boo 
						JOIN DSI_Subjects_Books SB ON SB.Book_ID=Boo.Book_ID 
						JOIN DSI_Subjects Sub ON Sub.Sub_ID=SB.Sub_ID
						WHERE Sub.Sub_Term ="Text Mining"';
			$query = mysqli_query($db, $sql);
			if (!$query) {
				die ('SQL Error: ' . mysqli_error($db));
			}
          ...
          <tr>
			...
			$sql = 'SELECT Boo.* FROM DSI_Books Boo 
						JOIN DSI_Subjects_Books SB ON SB.Book_ID=Boo.Book_ID 
						JOIN DSI_Subjects Sub ON Sub.Sub_ID=SB.Sub_ID
						WHERE Sub.Sub_Term ="Data Science"';
			$query = mysqli_query($db, $sql);
			if (!$query) {
				die ('SQL Error: ' . mysqli_error($db));
			}
